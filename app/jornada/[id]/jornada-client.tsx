...
function isMatchDone(m: Match, pred: Prediction | undefined): boolean {
  if (m.special === 'exact') return pred?.exact_home != null && pred?.exact_away != null;
  return !!pred?.pick;
}

// ── Resultado de apuesta normal / doble ──────────────────────────────────────
function FinishedPickDetail({ pred, homeScore, awayScore, special }: {
  ...
  const hitColor  = isPickHit ? 'var(--fv-win)' : 'var(--fv-loss)';
  const ptsColor  = pts > 0 ? 'var(--fv-win)' : pts < 0 ? 'var(--fv-loss)' : 'var(--fv-tertiary)';
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <div style={{
          ...
          color: hitColor, lineHeight: 1, minWidth: 18, textAlign: 'center',
        }}>
          {pred.pick ?? '—'}
        </div>
        <div>
          <div style={{
            fontFamily: 'var(--fv-mono)', fontSize: 8, letterSpacing: '0.14em',
            textTransform: 'uppercase', color: 'var(--fv-muted)', marginBottom: 3,
          }}>
            Mi apuesta{isDouble ? ' · ×2' : ''}
          </div>
          <div style={{ fontFamily: 'var(--fv-sans)', fontSize: 12, fontWeight: 600, color: hitColor, lineHeight: 1 }}>
            {pred.pick ? PICK_LABEL[pred.pick] : '—'}
          </div>
        </div>
      </div>
      <div style={{ textAlign: 'right' }}>
        <div style={{
          fontFamily: 'var(--fv-mono)', fontSize: 8, letterSpacing: '0.12em',
          textTransform: 'uppercase', color: hitColor, marginBottom: 3,
        }}>
...
function FinishedExactDetail({ pred, homeScore, awayScore }: {
  ...
  const pts         = pred.points ?? 0;
  const statusColor = isExactHit ? 'var(--fv-flash)' : isPickHit ? 'var(--fv-win)' : 'var(--fv-loss)';
  const statusLabel = isExactHit ? '★ Exacto' : isPickHit ? '✓ 1X2' : '✗ Fallado';
  const ptsColor    = pts > 0 ? statusColor : pts < 0 ? 'var(--fv-loss)' : 'var(--fv-tertiary)';
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'right' }}>
        <div style={{
          fontFamily: 'var(--fv-mono)', fontSize: 8, letterSpacing: '0.12em',
          textTransform: 'uppercase', color: statusColor, marginBottom: 3,
        }}>
          {statusLabel}
        </div>
        <div style={{
          fontFamily: 'var(--fv-mono)', fontSize: 22, fontWeight: 800,
          lineHeight: 1, color: ptsColor,
        }}>
          {pts > 0 ? `+${pts}` : pts}
        </div>
      </div>
...
export function JornadaClient({
  ...
  const earnedPts = finishedMatches.reduce((s, m) => s + (predictions[m.id]?.points ?? 0), 0);
  ...
  return (
    <div className="fv-page" style={{ paddingBottom: 'calc(140px + env(safe-area-inset-bottom, 0px))', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div className="fv-scroll-x" style={{ display: 'flex', gap: 4, padding: '10px 16px' }}>
          {jornadasMeta.map(meta => {
            ...
            const bg     = isCurrent ? (isLive ? 'var(--fv-live-soft)' : 'var(--fv-accent-soft)') : 'transparent';
            const color  = isCurrent
              ? (isLive ? 'var(--fv-live)' : 'var(--fv-accent)')
              : isPast ? 'var(--fv-tertiary)' : isFuture ? 'oklch(0.35 0.01 150)' : 'var(--fv-muted)';
            ...
            return (
              <button
                ...
                style={{
                  ...
                  padding: '6px 12px', borderRadius: 999, background: bg, border, cursor: 'pointer',
                  fontFamily: 'var(--fv-mono)', fontSize: 11, fontWeight: 700,
                  letterSpacing: '0.06em', color, transition: 'background 0.15s, color 0.15s',
                  opacity: isFuture ? 0.45 : 1,
                }}
              >
                {isLive && <span className="fv-live-dot" style={{ width: 6, height: 6 }} />}
                {pillLabel(meta)}
                {isPast && !isCurrent && <span style={{ fontSize: 9, opacity: 0.7 }}>✓</span>}
              </button>
            );
          })}
        </div>
      ...
      {/* ── Resumen de jornada (cuando hay resultados) ── */}
      {jornadaSomeFinished && (
        <div className="glass" style={{
          margin: '14px 20px 0', padding: '16px 18px', borderRadius: 18,
          borderLeft: `3px solid ${earnedPts > 0 ? 'var(--fv-accent)' : earnedPts < 0 ? 'var(--fv-loss)' : 'var(--fv-line)'}`,
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          <div>
            <div style={{
              fontFamily: 'var(--fv-mono)', fontSize: 9, letterSpacing: '0.14em',
              textTransform: 'uppercase', color: 'var(--fv-muted)', marginBottom: 6,
            }}>
              {jornadaAllFinished ? 'Resultado final' : `${finishedMatches.length}/${matches.length} jugados`}
            </div>
            <div style={{
              fontFamily: 'var(--fv-serif)', fontSize: 36, letterSpacing: '-0.03em', lineHeight: 1,
      ...
      {groups.length > 1 && (
        <div className="fv-scroll-x" style={{ padding: '14px 20px 0', display: 'flex', gap: 6 }}>
          {groups.map(g => {
            ...
            const gPts      = g.matches.reduce((s, m) => s + (predictions[m.id]?.points ?? 0), 0);
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: (gFinished ? gPts > 0 : gDone) ? 'var(--fv-accent-soft)' : 'var(--fv-surface-2)',
                  color: (gFinished ? gPts > 0 : gDone) ? 'var(--fv-accent)' : 'var(--fv-muted)',
                  fontFamily: 'var(--fv-mono)', fontSize: 11, fontWeight: 700,
              ...
              >
                {letter}
                {gFinished ? ` ${gPts > 0 ? '+' : ''}${gPts}` : gDone ? ' ✓' : ` ${gFilled}/${g.matches.length}`}
              </button>
            );
          })}
        </div>
      ...
      <div style={{ padding: '20px 20px 0', display: 'flex', flexDirection: 'column', gap: 28 }}>
        {groups.map(group => {
          ...
          const gPts   = group.matches.reduce((s, m) => s + (predictions[m.id]?.points ?? 0), 0);
            ...
            <div key={group.slug} id={group.slug} style={{ scrollMarginTop: 20 }}>
              ...
              <div style={{
                ...
                {gAllFinished ? (
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ color: 'var(--fv-line)' }}>·</span>
                    <span style={{ color: 'var(--fv-tertiary)' }}>{gHits}/{group.matches.length}</span>
                  </span>
                ) : (
                  <span style={{ color: gFilled === group.matches.length ? 'var(--fv-accent)' : 'var(--fv-tertiary)' }}>
              ...
              <div className="fv-match-grid">
                {group.matches.map(m => {
                  ...
                  // myPickHit drives border color only — no center badge (avoids duplicate content)
                  ...
                  return (
                    <MatchCard
                      ...
                      myPickHit={isFinished ? myPickHit : undefined}
                    >
                      {isFinished ? (
                        /* ── Finished state: show result detail ── */
                        pred && isExact ? (
                          <FinishedExactDetail
                            pred={pred}
      ...
      <div className="fv-progress-float" style={{
        ...
        {jornadaSomeFinished ? (
          <>
            {/* Dots: green=hit, red=miss, gray=pending/zero */}
            <div style={{ display: 'flex', gap: 2, flex: 1, flexWrap: 'wrap' }}>
              {matches.map(m => {
                ...
                const bg   = isF
                  ...
                  : 'var(--fv-surface-2)';
                return <div key={m.id} style={{ width: 6, height: 6, borderRadius: 3, flexShrink: 0, background: bg, transition: 'background 0.2s' }} />;
              })}
            </div>
            <div style={{
              fontFamily: 'var(--fv-mono)', fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap',
              letterSpacing: '0.06em',
